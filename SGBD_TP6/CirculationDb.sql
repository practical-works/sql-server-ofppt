...
select * from Voiture;
select * from Voiture, Personne where Personne.CIN = Voiture.CIN;
select * from Accident;

--1.Développer les vues suivantes :
...
--9. Afficher les noms des propriétaires de voiture qui résident dans une ville où il y a eu plus de 1000 accidents.
select nom from Personne where ville_p in (
		select ville_Acc from Accident group by ville_Acc having count(*)>1
);

--10. Afficher le nom des propriétaires d’une voiture accidentée, 
-- qui résident dans une ville où il y a eu plus de 1000 accidents.
